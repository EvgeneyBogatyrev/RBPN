...
parser.add_argument('--model', default='/model/weights/RBPN_4x.pth', help='sr pretrained base model')

opt = parser.parse_args()

gpus_list=range(opt.gpus)
print(opt)
number_of_frames = len(os.listdir(opt.data_dir))
...
def save_img(img, img_name, pred_flag):
    ...
    # save img
    save_dir=os.path.join(opt.output, list(opt.data_dir.split('/'))[-1])
    ...
    save_fn = save_dir +'/frame'+ str(int(img_name)).zfill(4)+'.png'
